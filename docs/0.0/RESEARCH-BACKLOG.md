...
| [PR-026](../../prs/PR-026-content-specific-vision-prompt.md) | `design-research ✗` | **Content-specific vision prompt — NEXT UP.** The shipped `prompts/vision.txt` (PR-015, CRISPE) was written for TED talks and film clips: 9 of 10 instructions presuppose humans, faces and camera work. Measured on 13 corpus videos: 76% of visual segments carry absent-human boilerplate, 61% restate that nothing changed. Prompt-induced hallucination from instruction presupposition is proven on the Qwen family (arXiv:2604.21911, 94.1% → 56.7%). Separately, the model narrates TradingView's hovered-candle header as a price time series — a fabricated chronology the fidelity metric scores as *supported*. Carries a **Phase 6 (Iterative Empirical Tuning)** with a stopping rule and held-out split fixed in Phase 2. Blocking sub-question: what is the primary measure, given PR-020 Phase 5.5 and PR-023 calibration both limit the existing metrics. |
